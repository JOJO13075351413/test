...
return view.extend({
    load: function () {
        return Promise.all([
            loadCssOnce('/luci-static/custom/css/log/kernal.css')
        ]);
    },
    ...
    render: function () {
        ...
        const bar = el('div', { class: 'klog-bar cbi-section' }, [
            el('div', { class: 'toolbar-hsplit' }, [
                ...
                el('div', { class: 'controls-right' }, [
                    ...
                    el('a', { id: 'btnExport', class: 'ns-icon-btn ns-export', href: '#', download: 'Kernel_Log.txt', title: __('导出') }, [
                        el('span', { class: 'ico' })
                    ]),
                ]),
            ])
        ...
        const pager = el('div', { class: 'cbi-section-footer' }, [
            el('div', { class: 'pager-modern' }, [
                el('div', { class: 'pm-left' }, [el('span', { id: 'totalInfo' }, __('共 0 行'))]),
        ...
        async function fetchLogs() {
            const txt = await fs.exec_direct('/bin/dmesg', ['-r']).catch(err => {
                throw new Error(err && err.message || 'dmesg failed');
            });
