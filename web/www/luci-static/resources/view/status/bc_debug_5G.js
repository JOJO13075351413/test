...
return view.extend({
		...
		return uci.load('uhttpd').then(function() {
			...
			if ( '0' === debug_disabled ){
				return Promise.all([
					...
					L.resolveDefault(fs.exec_direct("/usr/sbin/iw", [ 'wlan0', 'station', 'dump' ]), null),
					fs.trimmed('/sys/kernel/debug/ieee80211/phy0/bcwifi/txq'),
					fs.trimmed('/sys/kernel/debug/ieee80211/phy0/bcwifi/hwq'),
					fs.trimmed('/sys/kernel/debug/ieee80211/phy0/bcwifi/stats'),
					fs.trimmed('/sys/kernel/debug/ieee80211/phy0/bcwifi/lmactx'),
					fs.trimmed('/sys/kernel/debug/ieee80211/phy0/bcwifi/lmacrx'),
	...
	handleDownload: function(rpc_replies) {
		var cmds = ['ifconfig', 'iwinfo', 'iw wlan0 station dump', 'txq', 'hwq', 'stats', 'lmactx', 'lmacrx', 'clk_summary'];
	...
	render: function(rpc_replies) {
		if ( 3 == rpc_replies.length ){
			...
			m = new form.JSONMap(mapdata, _('5G Debug Info'));
			m.tabbed = true;
			...
			s = m.section(form.NamedSection, 'iw_station_sec', 'iw wlan0 station dump');
